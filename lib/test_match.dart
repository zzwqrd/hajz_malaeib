...
class TestScreenTime extends StatefulWidget {
  @override
  _TestScreenTimeState createState() => _TestScreenTimeState();
}

class _TestScreenTimeState extends State<TestScreenTime> {
  ...
  void _onDateChanged(String date) {
    ...
    Future.delayed(Duration(seconds: 1), () {
      ...
      // ترتيب الأحداث بحيث يكون العنصر الأول هو التاريخ المحدد
      newDisplayedEvents.sort((a, b) {
        DateTime dateA = DateTime.parse(a['date']);
        DateTime dateB = DateTime.parse(b['date']);
        if (dateA.isAtSameMomentAs(selectedDate)) return -1;
        if (dateB.isAtSameMomentAs(selectedDate)) return 1;
  ...
  List<DateItem> _generateDateRange() {
    ...
    for (int i = -6; i <= 7; i++) {
      ...
      generatedDates.add(DateItem(dayName, day, fullDate));
    }
